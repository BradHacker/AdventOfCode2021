...
fn main() {
    ...
    for number in bingo_inputs {
        ...
        for i in 0..boards.len() {
            end = mark_number(boards[i].to_owned(), &mut marker_boards[i], number);
            if end {
    ...
    let unmarked_sum = sum_unmarked(boards[winning_board_index].to_owned(), marker_boards[winning_board_index].to_owned());

    println!("Answer is {}", winning_num * unmarked_sum);

    // println!("{:?}", boards);
    // println!("{:?}", marker_boards);
}
